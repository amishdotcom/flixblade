 ...
 * 
 * Amish Dotcom Notice - This Script saved me a lot of time and also helped me to master the search regex, Thanks to Ben Griffiths
 */
class Pagination
{
    ...
    */
    ...
    public function build_li_element($class, $text, $is_link = false, $page_number = 0)
    {
        ...
        if($is_link == false OR $page_number == $this->current_page)
        {
            $this->links_html .= '<li class="'.$class.'"><span>'.$text.'</span></li>'.PHP_EOL;
        }
        else
        {
            $this->links_html .= '<li class="'.$class.'"><a href="'.$GLOBALS['search_page_loc'].'?q='.$GLOBALS['get_query'].''.$this->build_link_url($page_number).'">'.$text.'</a></li>'.PHP_EOL;
        }
    }
    ...
    protected function build_links_next_last()
    {
        ...
        if($this->options['show_links_prev_next'] == true)
        {
            ...
            {
                $this->links_html .= '<li class="'.$this->options['class_live_links'].'"><a href="'.$GLOBALS['search_page_loc'].'?q='.$GLOBALS['get_query'].''.$this->build_link_url($this->current_page + 1).'">'.$this->options['text_next'].'</a></li>'.PHP_EOL;
            }
        ...
        if($this->options['show_links_first_last'] == true)
        {
            ...
            elseif($this->current_page != $this->total_pages)
            {
                $this->links_html .= '<li class="'.$this->options['class_live_links'].'"><a href="'.$GLOBALS['search_page_loc'].'?q='.$GLOBALS['get_query'].''.$this->build_link_url($this->total_pages).'">'.$this->options['text_last'].'</a></li>'.PHP_EOL;
            }
